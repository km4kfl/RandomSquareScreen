...
static void UpdateScreen(HWND hwnd, HDC hdc, PAINTSTRUCT &ps) {
    ...
    SmartGdiObj sgdi0(SelectObject(mem_hdc.GetHandle().hdc, hbm_win.GetHandle()));
    ...
    //SetDIBitsToDevice(hdc, 0, 0, width, height, 0, 0, 0, bmp.bmHeight, p_data, (BITMAPINFO*)&bmp_hdr, DIB_RGB_COLORS);
    ///*
    int r6 = SetDIBits(
    ...
    SmartGdiObj sgdi1(SelectObject(mem_hdc.GetHandle().hdc, hbm_tmp.GetHandle()));
